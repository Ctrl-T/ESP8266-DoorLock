...
bool Finger::checkOpenInstr() {
    ...
    if (recvBuff[8] != 0x00) {
        DEBUG_PRINTLN("获取指纹失败");
        return false;
    }
    // 生成指纹特征并验证是否匹配已录入的指纹
    sendCmd(CMD_GENERATE_0);
    if (recvBuff[8] != 0x00) {
        DEBUG_PRINTLN("生成指纹失败");
        return false;
    }
