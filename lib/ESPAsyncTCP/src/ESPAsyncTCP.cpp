...
#endif
      ,
      _pcb_sent_at(0), _close_pcb(false), _ack_pcb(true), _tx_unacked_len(0),
      _tx_acked_len(0), _tx_unsent_len(0), _rx_ack_len(0), _rx_last_packet(0),
      _rx_since_timeout(0), _ack_timeout(ASYNC_MAX_ACK_TIME), _connect_port(0),
...
void AsyncClient::_close() {
    if (_pcb) {
        ...
        if (ERR_OK == err) {
            ...
            ASYNC_TCP_DEBUG(
                "_close[%u]: abort() called for AsyncClient 0x%" PRIXPTR "\n",
...
void AsyncClient::_sent(std::shared_ptr<ACErrorTracker> &errorTracker,
    ...
    if (_tx_unacked_len == 0) {
        ...
        if (_sent_cb) {
            _sent_cb(_sent_cb_arg, this, _tx_acked_len,
                     (millis() - _pcb_sent_at));
...
void AsyncClient::_recv(std::shared_ptr<ACErrorTracker> &errorTracker,
    ...
    if (NULL == pcb || ERR_OK != err) {
        ...
        // mainloop, not from interrupt context. You can allocate a Packet
        // buffers (PBUF) in interrupt context and put them into a queue which
        // is processed from mainloop." And the description of "Mainloop Mode"
        // option 2:
        //    https://www.nongnu.org/lwip/2_1_x/pitfalls.html
        // "2) Run lwIP in a mainloop. ... lwIP is ONLY called from mainloop
        // callstacks here. The ethernet IRQ has to put received telegrams into
        // a queue which is polled in the mainloop. Ensure lwIP is NEVER called
        // from an interrupt, ...!" Based on these comments I am thinking
        // tcp_recv_fn() is called from somebody's mainloop(), which could only
        // have been reached from a delay like function or the Arduino sketch
        // loop() function has returned. What I don't want is for the client
        // sketch to delete the AsyncClient object via _error() while it is in
        // the middle of using it. However, the client sketch must always test
        // that the connection is still up at loop() entry and after the return
        // of any function call, that may have done a delay() or yield().
        _error(err);
        return;
    }
...
void AsyncClient::keepAlive(uint16_t idle_sec, uint16_t intv_sec,
                            uint8_t count) {
...
err_t AsyncServer::_accept(tcp_pcb *pcb, err_t err) {
    ...
    if (NULL == pcb || ERR_OK != err) {
        ...
        // An error code if there has been an error accepting. Only return
        // ERR_ABRT if you have called tcp_abort from within the callback
        // function! eg. 2.1.0 could call with error on failure to allocate pcb.
    ...
    if (_connect_cb) {
        ...
        if (_ssl_ctx) {
            if (tcp_ssl_has_client() || _pending) {
                struct pending_pcb *new_item =
                    (struct pending_pcb *)malloc(sizeof(struct pending_pcb));
                if (!new_item) {
                ...
                if (c) {
                    ...
                    ASYNC_TCP_DEBUG("_accept[_ssl_ctx]: new AsyncClient() "
